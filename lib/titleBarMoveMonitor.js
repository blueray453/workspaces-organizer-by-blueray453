...
export class TitleBarMoveMonitor {
    ...
    // no delay.
    //
    // Also builds the icon by hand and settles it in, the same way
    // WindowReorderDragController.performCrossWorkspaceDrop() does for
    // the DND-based reorder path — going through the generic
    // ThumbnailDisplayModeController._sync() rebuild instead just
    // silently add_child()s the new icon with no animation at all.
    _switchWindowToThumbnail(thumb, window) {
        ...
        if (willStayDirect) {
            ...
            // Crossing the direct/collection threshold — no individual
            // icon to animate, fall back to the normal rebuild into the
            // "N windows" overflow button.
