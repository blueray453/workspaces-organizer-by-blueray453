...
export class AppSearchOverlay {
    ...
    _buildUI() {
        ...
        // Self-delegate, same pattern as every other reactive actor in
        // this codebase (WorkspaceThumbnail, WindowIconButton, etc). All
        // drag feedback funnels through _handlePinnedDragOver /
        // _acceptPinnedDrop below — whether the drop lands on empty bar
        // space or directly on another icon, the logic is identical.
    ...
    _renderPinnedBar() {
        ...
        for (const app of favorites) {
            ...
            // Lines up with InsertionPlaceholder's sizeHint fallback so
            // the ghost placeholder matches this icon's actual size.
            button.icon_size = PINNED_ICON_SIZE;

            // ---- Hover tooltip (name) — shares FloatingTooltip with the
            // workspace name hint and the drag insertion hint below.
            const tooltip = new FloatingTooltip({ parent: this._container });
            ...
            // Clean up the tooltip actor when the button is destroyed (bar re-render, etc.)
            ...
            // ---- Drag source ----
            // Same semi-transparent + bordered look as window-icon
            // reorder drags (.reorder-drag-source), applied to the actual
            // flying actor via dragActorOpacity, not a plain icon.
            ...
            draggable.connect('drag-end', () => {
                this._pinnedDragActive = false;
                button.remove_style_class_name('reorder-drag-source');
            ...
            // ---- Drop target ----
            // Forwards to the exact same insertion logic the bar itself
            // uses, so hovering an icon vs. hovering empty bar space
            // gives identical feedback.
    ...
    // Single entry point for "a drag is hovering the pinned bar" — called
    // identically whether the pointer is over empty bar space or over
    // another pinned icon. Shows the ghost placeholder plus a
    // "Move before/after X" hint.
    _handlePinnedDragOver(source) {
        ...
        // Match WorkspaceThumbnail.handleDragOver: always read the real
        // pointer position rather than trusting the x/y dnd.js passes in.
        // That parameter isn't guaranteed to share a coordinate space
        // between the bar's own handleDragOver (empty space) and each
        // button's forwarded handleDragOver (icon-to-icon) — mixing the two
        // is what was causing the insertion index to flip back and forth.
        ...
        if (!this._pinnedDragHint)
            this._pinnedDragHint = new FloatingTooltip({ parent: this._container });
    ...
    _acceptPinnedDrop(source, actor) {
        ...
        // _renderPinnedBar() (triggered by the favorites 'changed' signal
        // moveFavoriteToPos fires) rebuilds every pinned icon from scratch,
        // so the flying drag actor is now an orphan — dispose it rather than
        // leaving it parented at the top of the stage.
        if (actor) {
            if (actor.get_parent())
