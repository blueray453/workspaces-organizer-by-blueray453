...
export class AppSearchOverlay {
    ...
    _buildUI() {
        ...
        this._pinnedBarBox._delegate = this;
        this._pinnedBarBox.handleDragOver = (source) => {
            if (!source._appId) return DND.DragMotionResult.NO_DROP;
            return DND.DragMotionResult.MOVE_DROP;
        };
        this._pinnedBarBox.acceptDrop = (source) => {
            if (!source._appId) return false;
            this._reorderFavorite(source._appId, null, true); // null target = end
            return true;
        };
    ...
    _renderPinnedBar() {
        ...
        for (const app of favorites) {
            ...
            const tooltip = new St.Label({
                text: name,
                style_class: 'dmenu-pinned-tooltip',
                visible: false,
            });

            // Parented to the overlay container so it can float above other icons
            this._container.add_child(tooltip);
            ...
            button.connect('notify::hover', () => {
                ...
                if (button.hover) {
                    this._showPinnedTooltip(button, tooltip);
                } else {
                    tooltip.hide();
                }
            });
            ...
            draggable.connect('drag-begin', () => {
                ...
                button.add_style_class_name('dmenu-pinned-dragging');
                button._tooltip?.hide();
            });
            draggable.connect('drag-end', () => {
                this._pinnedDragActive = false;
                button.remove_style_class_name('dmenu-pinned-dragging');
            });
            button._draggable = draggable;

            // ---- Drop target (reorder relative to this icon) ----
            button._delegate = button;
            button.handleDragOver = (source) => {
                if (!source._appId || source._appId === id)
                    return DND.DragMotionResult.NO_DROP;
                return DND.DragMotionResult.MOVE_DROP;
            };
            button.acceptDrop = (source, actor, x) => {
                if (!source._appId || source._appId === id)
                    return false;
                const [buttonX] = button.get_transformed_position();
                const insertAfter = (x - buttonX) > button.width / 2;
                this._reorderFavorite(source._appId, id, insertAfter);
                return true;
            };
    ...
    _showPinnedTooltip(button, tooltip) {
        tooltip.show();
        // Let St compute its natural size before positioning
        const [tooltipWidth, tooltipHeight] = tooltip.get_size();
        const [buttonX, buttonY] = button.get_transformed_position();
        const buttonWidth = button.width;

        const x = Math.round(buttonX + (buttonWidth - tooltipWidth) / 2);
        const y = Math.round(buttonY - tooltipHeight - 6); // 6px gap above icon
    ...
    _reorderFavorite(sourceId, targetId, insertAfter) {
        if (sourceId === targetId) return;

        const favorites = this._favorites.getFavorites();
        let ids = favorites.map(fav => fav.get_id()).filter(id => id !== sourceId);

        let insertIndex;
        if (targetId === null) {
            // Dropped on empty bar space -> move to end
        ...
        try {
            this._favorites.moveFavoriteToPos(sourceId, insertIndex);
        } catch (e) {
