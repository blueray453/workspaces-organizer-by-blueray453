...
// Pinned bar size (make these as large as you want)
const PINNED_ICON_SIZE = 96;       // icon size in pixels
const PINNED_BAR_HEIGHT = 120;     // total bar height (icon + padding)
...
export class AppSearchOverlay {
    ...
    // -------------------- Safe app enumeration (fixes crash) --------------------
    _getAllApps() {
        ...
        // Fallback to Gio.AppInfo if nothing found
        if (appInfos.length === 0) {
            try {
                const gioApps = Gio.AppInfo.get_all().filter(app => {
                    try { return app.should_show(); } catch { return false; }
                });
                // Convert to Shell.App if possible (for favorites, launch)
        ...
        for (const app of favorites) {
            ...
            button.connect('clicked', () => {
                try {
                    ...
                    journal(`[AppSearchOverlay] Failed to launch pinned app: ${e.message}`);
                }
                this._close();
            });

            // Right-click: context menu (like gdmenu)
    ...
    _togglePin(item) {
        ...
        // _onFavoritesChanged() will refresh everything
    }
