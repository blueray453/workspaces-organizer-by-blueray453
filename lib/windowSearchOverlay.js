...
export class WindowSearchOverlay extends SearchOverlayBase {
    ...
    // Nemo windows sharing the same title are treated as duplicates (e.g.
    // two windows open on the same folder) — only the most recently used
    // one is kept. file_progress windows (copy/move progress dialogs) are
    // never deduplicated or matched against anything; every one of them
    // is always shown.
    // Nemo windows sharing the same title are treated as duplicates (e.g.
    // two windows open on the same folder) — the older one (by user_time)
    // is closed outright, not just hidden from the results. file_progress
    // windows (copy/move progress dialogs) are never touched or matched
    // against anything; every one of them is always shown and kept open.
    _dedupeNemoWindows(windows) {
        ...
        for (const w of windows) {
            ...
            if (w.get_user_time() > existing.get_user_time()) {
                // New window is more recent — close the one we kept before,
                // swap it in.
                ...
                // Existing window is more recent — close the new one instead.
