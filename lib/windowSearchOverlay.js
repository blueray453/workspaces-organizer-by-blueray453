...
export class WindowSearchOverlay extends SearchOverlayBase {
    // `focusedWindow`: which window should be shown as "currently
    // focused" — computed by overlayService (via FocusHistory for the
    // workspace-scoped case, since workspace.activate() does not
    // reliably refocus a window on its own). Falls back to the live
    // value if the caller didn't have an answer.
    constructor(windows, settings, focusedWindow = null) {
        super(settings);
        journal(`[WindowSearchOverlay] Opening with ${windows.length} windows`);
        this._windows = windows;
        this._focusedWindow = focusedWindow ?? global.display.focus_window;
        this._buildUI();
    ...
    _getAllResultsSorted() {
        ...
        // Float the currently-focused window to the top, on top of the
        // alphabetical sort above. Keeps everything else's relative order
        // untouched — this is a simple move-to-front, not a new sort key.
        if (this._focusedWindow) {
            const focusedIndex = items.findIndex(item => item.window === this._focusedWindow);
            if (focusedIndex > 0) {
                const [focusedItem] = items.splice(focusedIndex, 1);
                items.unshift(focusedItem);
            }
        }
    ...
    _buildResultRow(item, index) {
        ...
        if (item.window === this._focusedWindow)
            button.add_style_class_name('collection-result-item-focused');
    ...
    // Called on open (rows already marked correctly in _buildResultRow,
    // using this._focusedWindow) and again via _onFocusWindowChanged()
    // whenever WM focus changes while the overlay stays open.
    _updateFocusedHighlight() {
        this._results.forEach((item, index) => {
            const button = this._resultButtons[index];
            if (!button) return;
            if (item.window === this._focusedWindow)
                button.add_style_class_name('collection-result-item-focused');
            else
                button.remove_style_class_name('collection-result-item-focused');
        });
    }

    // IMPORTANT: only adopt the live focus-window if it's actually one
    // of the windows THIS overlay is showing. workspace.activate() does
    // not reliably/immediately refocus a window (see overlayService's
    // FocusHistory), so a notify::focus-window can still fire later for
    // reasons unrelated to "which window in this list is focused" —
    // e.g. focus settling to null momentarily, or moving to a window on
    // a different workspace than the one this overlay represents.
    // Blindly adopting that would incorrectly clear a correct initial
    // highlight.
    _onFocusWindowChanged() {
        if (this._closed) return;
        const liveFocus = global.display.focus_window;
        if (!liveFocus)
            return; // irrelevant/transient — keep showing our current highlight
        const isOneOfOurs = this._results.some(item => item.window === liveFocus);
        if (!isOneOfOurs)
            return; // focus moved somewhere outside this overlay's list — ignore
        this._focusedWindow = liveFocus;
        this._updateFocusedHighlight();
    }
