...
// One "label centered above/below an actor" tooltip. Used for the
// workspace drag-over name hint, the pinned-app hover tooltip, and the
// pinned-app drag insertion hint — every floating label in the extension
// goes through this so they look and behave identically.
export class FloatingTooltip {
    ...
    // Position the label centered above `actor`, flipping below if there
    // isn't room above. Used for the drag-insertion hint, where staying
    // clear of the pointer/dragged icon above the bar matters more than
    // a fixed side.
    showAbove(actor, text, gap = 8) {
        this._show(actor, text, gap, false);
    }

    // Position the label centered below `actor`, flipping above if there
    // isn't room below. Used for the pinned-app hover tooltip, where the
    // name should sit just under the icon rather than floating above it.
    showBelow(actor, text, gap = 8) {
        this._show(actor, text, gap, true);
    }

    _show(actor, text, gap, preferBelow) {
        ...
        const above = Math.round(actorY - height - gap);
        const below = Math.round(actorY + actorHeight + gap);

        let y;
        if (preferBelow) {
            y = below;
            if (y > maxY) y = above; // not enough room below — flip above
        } else {
            y = above;
            if (y < minY) y = below; // not enough room above — flip below
        }
