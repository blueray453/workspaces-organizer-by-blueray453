...
// The big live-clone preview that pops up above the panel when hovering a
// window icon (and CTRL isn't held). Owns its own actor lifecycle and
// nothing else. `onHoverChange` fires with (isHovered: boolean) whenever
// the preview actor itself is hovered/unhovered, so the owner can decide
// whether to keep it open.
//
// show() is a no-op if already showing — calling it again on a preview
// that's already open must NOT rebuild the clone actor. Rebuilding here
// was previously the cause of a visible flicker when the icon was
// re-hovered while a preview was still fading out from a prior hover.
