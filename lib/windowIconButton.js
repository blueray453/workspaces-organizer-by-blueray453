...
import { DragMotionResult, getDraggedWindow, createReorderDraggable } from './dragHelpers.js';
import { createContextMenu, PopupSeparatorMenuItem } from './menuHelpers.js';
...
// polling only runs while one is active.
class ActivePreviewTracker {
    static activePreview = null;
    static _ctrlPollId = null;
    static _ctrlPressed = false;

    static registerPreview(preview) {
    ...
    static unregisterPreview(preview) {
...
export class WindowIconButton extends St.Button {
    ...
    constructor(window, settings) {
        ...
        this._delegate = this;
        this._draggable = createReorderDraggable(this, this.icon_size);

        // Drag lifecycle: mark ourselves as the ghost template for the
        // duration of the drag. Everything else (drawing the ghost,
        // computing insert index, executing the drop) happens in the
        // thumbnail that the pointer is over.
        this._draggable.connect('drag-begin', () => {
            DragSession.ghostTemplate = this;
            this.add_style_class_name('reorder-drag-source');
        });
        this._draggable.connect('drag-end', () => {
            DragSession.ghostTemplate = null;
            WorkspaceThumbnailRegistry.hideAllGhosts();
            WorkspaceThumbnailRegistry.hideAllNameHints();
            try { this.remove_style_class_name('reorder-drag-source'); }
    ...
    // ==================== DnD FORWARDING ====================
    // This icon is not a classifier. It receives DnD events because the
    // pointer happens to be over it, but the thumbnail behind it decides
    // what a drag is and what to do. The only thing this icon contributes
    // is the per-icon "hover for 400 ms to auto-raise the window"
    // behavior, which is genuinely local to the icon.
    // ==========================================================
    ...
    handleDragOver(source, actor, x, y, time) {
        ...
        if (!getDraggedWindow(source))
            this._dragActivator.notifyDragOver();

        const thumbnail = this._getThumbnail();
        if (thumbnail?.handleDragOver)
            return thumbnail.handleDragOver(source, actor, x, y, time);
        return DragMotionResult.CONTINUE;
    }

    acceptDrop(source, actor, x, y, time) {
        const thumbnail = this._getThumbnail();
        if (thumbnail?.acceptDrop)
            return thumbnail.acceptDrop(source, actor, x, y, time);
        return false;
    }
    ...
    destroy() {
        ...
        // If we were mid-drag as the source, drop the session.
        if (DragSession.ghostTemplate === this) {
            DragSession.ghostTemplate = null;
            WorkspaceThumbnailRegistry.hideAllGhosts();
            WorkspaceThumbnailRegistry.hideAllNameHints();
        }
