...
export function showWindowOverlay(windows, context = null) {
    if (!_settings) {
        journal('[overlayService] showWindowOverlay before init — ignoring', true);
        return false;
    }
    SearchOverlayBase.closeCurrent();
    const overlay = new WindowSearchOverlay(windows, _settings);
...
export function showWorkspaceWindows(workspaceIndex) {
    ...
    return showWindowOverlay(windows, { type: 'workspace', workspaceIndex });
}
