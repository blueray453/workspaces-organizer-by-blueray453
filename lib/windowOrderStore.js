...
// a change callback. ThumbnailDisplayModeController reads this normally;
// WindowReorderDragController is the ONLY external caller of
// setSuppressSync()/insertWindowImmediate() — those exist specifically to
// let it perform an atomic cross-thumbnail transplant without triggering
// two independent rebuild cycles mid-drop.
export class WindowOrderStore {
    ...
    // Internal to the WindowReorderDragController transplant flow ONLY.
    // Do not call from thumbnail/display-mode code — those should react
    // to order changes via setOnOrderChanged(), not bypass them.
    setSuppressSync(suppress) {
    ...
    // Insert a window into `order` immediately, bypassing the async
    // add-window debounce. Used by WindowReorderDragController when
    // transplanting an already-existing preview across thumbnails, so the
    // store and the DOM agree before any signal handler gets a chance to
    // schedule a duplicate add.
    insertWindowImmediate(window, index) {
        ...
        this._pendingInsertIndices.delete(window);
        const idx = Math.max(0, Math.min(index, this._order.length));
        this._order.splice(idx, 0, window);
    }
}