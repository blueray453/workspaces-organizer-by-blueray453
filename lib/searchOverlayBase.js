...
export class SearchOverlayBase {
    // The overlay that's currently open, if any. Only one can be open at
    // a time (each pushes a full-screen modal grab), so one static is
    // enough. The getter self-clears once an overlay has marked itself
    // closed, which covers BOTH teardown paths: the normal _close(), and
    // WindowSearchOverlay._onResultDragBegin(), which sets _closed
    // directly and tears down WITHOUT going through _close().
    static _current = null;

    static get current() {
        if (SearchOverlayBase._current?._closed)
            SearchOverlayBase._current = null;
        return SearchOverlayBase._current;
    }

    static closeCurrent() {
        const overlay = SearchOverlayBase.current;
    ...
    _onBeforeClose() { }
    // Called whenever global.display's focus-window changes while this
    // overlay is open (right after we've reclaimed Clutter key focus for
    // the entry). Subclasses can use this to refresh a "currently
    // focused window" indicator without rebuilding the whole result list.
    ...
    _open() {
        ...
        SearchOverlayBase._current = this;

        // Mutter's own workspace-activate focus restoration can fire
        // AFTER this synchronous grab (it's tied to the switch-workspace
        // animation completing, not guaranteed to happen before this
        // point), stealing keyboard focus from the overlay entry back to
        // a window. Reclaim it whenever the WM's focused window changes
        // while this overlay stays open — cheap, idempotent, and correct
        // regardless of whether the workspace switch that triggered this
        // overlay happens before or after _open() is called. Also lets
        // subclasses refresh anything derived from "what's focused".
