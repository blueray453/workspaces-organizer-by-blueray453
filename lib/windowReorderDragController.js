...
export const WindowReorderDragController = {
    ...
    _acceptCrossWorkspace(targetThumbnail, draggedWindow, actor, insertIndex) {
        ...
        try {
            ...
            // ---- 1. Suppress sync on source only (target will rebuild after insert) ----
            ...
            // ---- 2. Move window to target monitor (if needed) ----
            ...
            if (sourceThumbnail) {
                ...
                if (preview) {
                    // We don't need it – destroy it to be safe
            ...
            // ---- 7. Always create a fresh icon for the target ----
            if (targetWillStayDirect) {
                journal('[WindowReorderDragController] creating new icon for target');
                ...
                // Collection mode – target will rebuild itself with an overflow button
                journal('[WindowReorderDragController] target switches to collection mode');
                // No actor to insert; sync will rebuild everything.
            }

            // ---- 8. Re‑enable sync on source ----
            if (sourceOrderStore) {
            ...
            // ---- 9. If target went into collection mode, force a rebuild ----
            if (!targetWillStayDirect) {
