...
export const WindowReorderDragController = {
    ...
    acceptDrop(targetThumbnail, source, actor, time) {
        ...
        // ---- External drag (text, files) ----
        // Workspace was already switched on hover in WorkspaceThumbnail.
        // We just accept so the DnD system doesn't try to find another target
        // and doesn't cancel the drag.
        if (!draggedWindow) {
            journal('[WindowReorderDragController] acceptDrop: external drop accepted');
            this.clearPlaceholder();
            this.endDrag();
            return true;
        }

        // ---- Internal window-icon reorder (unchanged) ----
        try {
        ...
        if (targetDisplay.mode === 'direct') {
            ...
            const result = this.computeInsertionFromPointer(
                draggedWindow, targetOrderStore.order, pointerX, targetBox);
