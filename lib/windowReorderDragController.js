...
export const WindowReorderDragController = {
    ...
    _acceptCrossWorkspace(targetThumbnail, draggedWindow, actor, insertIndex) {
        ...
        try {
            ...
            // ---- 1. Suppress sync on BOTH source and target ----
            targetOrderStore.setSuppressSync(true);
            ...
            // ---- 2. Move window to target monitor ----
            ...
            if (sourceThumbnail) {
                ...
                if (preview) {
                    ...
                    // Log map size after removal (add to releasePreview if needed)
                }
            } else {
            ...
            if (preview) {
                journal(`[WindowReorderDragController] preview is alive? ${!!preview} parent=${preview.get_parent()}`);
                try {
            ...
            // ---- 6. Check preview validity after workspace change ----
            if (preview) {
                try {
                    const stage = preview.get_stage();
                    if (stage === null) {
                        // Actor was destroyed during workspace change (likely by source sync)
            ...
            if (targetWillStayDirect) {
                if (preview) {
                    ...
                    journal('[WindowReorderDragController] calling settleIcon on preview');
                    settleIcon(preview);
                    journal('[WindowReorderDragController] settleIcon done on preview');
                } else {
                    // Preview was destroyed – create a brand new icon
            ...
            // Only call sync() if we went into collection mode (so the overflow button appears)
            if (!targetWillStayDirect) {
