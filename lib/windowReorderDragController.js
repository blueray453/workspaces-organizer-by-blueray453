    ...
    _acceptCrossWorkspace(targetThumbnail, draggedWindow, actor, insertIndex) {
        ...
        if (targetWillStayDirect) {
            if (preview) {
                ...
                // No preview – create a new icon ourselves. `actor` is the
                // dragged WindowIconButton itself (restoreOnSuccess: false
                // means dnd.js won't touch it again), but since we're not
                // reusing it here we must destroy it explicitly or it leaks
                // as an orphaned actor with live signal handlers.
