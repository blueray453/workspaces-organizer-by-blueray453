...
export class WorkspaceThumbnail extends St.Button {
    ...
    handleDragOver(source, actor, x, y, time) {
        ...
        // The Shell DnD system calls us for these too (X11 + Wayland both emit
        // dnd-position-change). We do not own the payload — we only switch the
        // workspace on hover so the user can then drop onto a window there.
        if (!draggedWindow) {
            ...
            if (this._workspace !== currentWs) {
                journal(`[WorkspaceThumbnail] External drag → switching to workspace ${this._workspace.index()}`);
