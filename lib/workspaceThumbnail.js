...
class ThumbnailDisplayModeController {
    ...
    adoptPreview(window, preview, index) {
        ...
        this._box.insert_child_at_index(preview, Math.max(0, Math.min(index, count)));
    }
}
...
export class WorkspaceThumbnail extends St.Button {
    ...
    // ---- drag-over workspace name hint ----
    // Shown while ANY drag (icon reorder, search-overlay result, or a
    // native title-bar move via TitleBarMoveMonitor) hovers this
    // thumbnail, so the person can tell which workspace they're about to
    // drop onto without having to read the thumbnail's window contents.
    showNameHint() {
        if (!this._settings.get_boolean('show-workspace-names'))
            return;
        if (!this._nameHintLabel) {
            this._nameHintLabel = new St.Label({ style_class: 'workspace-drag-name-hint' });
            Main.layoutManager.addChrome(this._nameHintLabel);
        }
        this._nameHintLabel.set_text(Meta.prefs_get_workspace_name(this._workspace.index()));
        const [x, y] = this.get_transformed_position();
        const width = this.get_width();
        const hintWidth = this._nameHintLabel.width;
        this._nameHintLabel.set_position(x + (width - hintWidth) / 2, y - this._nameHintLabel.height - 8);
        this._nameHintLabel.show();
    }

    hideNameHint() {
        if (this._nameHintLabel) {
            Main.layoutManager.removeChrome(this._nameHintLabel);
            this._nameHintLabel.destroy();
            this._nameHintLabel = null;
        }
    }
    ...
    handleDragOver(source, actor, x, y, time) {
        ...
        if (!draggedWindow) return DND.DragMotionResult.CONTINUE;
