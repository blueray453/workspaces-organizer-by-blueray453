...
import { showWindowOverlay, showWorkspaceWindows } from './overlayService.js';
...
// ==================== DRAG LEAVE POLL ====================
// The Shell has no per-actor "drag left" signal, and grabs suppress
// Clutter's enter/leave events during a DnD. So we poll: once a drag
// has been seen over any thumbnail, tick every 100 ms and check
// whether the pointer is still over one and whether the primary button
// is still held. When either stops, clear the hover state and stop.
//
// Started lazily from handleDragOver; self-terminating.
let _leavePollId = 0;

function _isPointerOverAnyThumbnail() {
    const [px, py] = global.get_pointer();
    for (const t of WorkspaceThumbnailRegistry.getAll()) {
        if (!t.get_stage()) continue;
...
function _onLeavePollTick() {
    const [, , mods] = global.get_pointer();
    const buttonHeld = (mods & Clutter.ModifierType.BUTTON1_MASK) !== 0;

    if (!buttonHeld || !_isPointerOverAnyThumbnail()) {
...
function _ensureLeavePoll() {
    if (_leavePollId) return;
    _leavePollId = GLib.timeout_add(GLib.PRIORITY_DEFAULT, 100, _onLeavePollTick);
}
...
// direct-mode threshold. Opens WindowSearchOverlay via the same
// showWorkspaceWindows() path as the keybinding/DBus entry points, so
// this button gets the same workspace-activate + live-focus-highlight
// behavior instead of a separate shortcut that skipped both.
class WindowOverflowButton extends St.Button {
    static { GObject.registerClass(this); }

    constructor(getWorkspaceIndexFn) {
        super({ style_class: 'workspace-thumbnail-collection-icon', reactive: true, track_hover: true, can_focus: true });
        this._getWorkspaceIndexFn = getWorkspaceIndexFn;
        ...
        this._clickedId = this.connect('clicked', () => showWorkspaceWindows(this._getWorkspaceIndexFn()));
    }
...
class ThumbnailDisplayModeController {
    ...
    _enterCollectionMode(count) {
        ...
        this._windowPreviews.clear();
        if (!this._collectionIcon) {
            this._collectionIcon = new WindowOverflowButton(() => this._orderStore.workspace.index());
...
export class WorkspaceThumbnail extends St.Button {
    ...
    get windows() { return this._orderStore.order.slice(); }
    get workspace() { return this._workspace; }
